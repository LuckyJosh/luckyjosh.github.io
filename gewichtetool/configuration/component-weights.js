...
window.addEventListener("message", (msg) => {
  console.log("Communication: App got a direct message from Articulate");
  console.log(`Message:\n${JSON.stringify(msg.data)}`);
  switch (msg.data.method) {
    case "init":
      console.log("Communication: Handling method='init'")

      CONFIGURATION = msg.data.configuration;
      COMMUNICATION.port = msg.ports[0];
      COMMUNICATION.index = msg.data.index;
      COMMUNICATION.port.onmessage = (msg) => {
        console.log("Communication: App got a message via MessageChannel port")
        console.log("Communication: State before handling of message in app:\n", COMMUNICATION);
        switch (msg.data.method) {
          case "get":
            console.log("Communication: Handling method='get'")
            timings.push(new Date().getTime());
            console.log('Communication: Timings (in ms) of communication in app:', timings);
            COMMUNICATION.response = msg.data.value;

            console.log("Communication: State after handling of message in app:\n", COMMUNICATION);
            break;
          default:
            console.log("Communication: The message (via MessageChannel) could not be handled by app\n", msg)
        }

        console.log("Communication: State after handling of message in app:\n", COMMUNICATION);
      }

      console.log("Communication: MessageChannel initialized")
      console.log("Communication: State after initialization:\n", COMMUNICATION);

      console.log("Communication: post message to get data from Articulate");
      timings.push(new Date().getTime());
      COMMUNICATION.port.postMessage({ method: "get", key: undefined })

      // start p5js
      console.log("App: Start app after a short delay");
      setTimeout(() => myp5 = new p5(sketch, "sketch"), 75);

      break;
    default:
      console.log("Communication: The message (direct) could not be handled by app\n", msg)
  }

});


//const CONFIG = window.location.pathname.split('/').slice(-2, -1);

const sketch =
  (p5Inst) => {
    p5Inst.getDivomathConfig = function () {
      ...
      if (CONFIGURATION != undefined) {
        ...
        return DivomathConfig()
      }

    };

    p5Inst.getDivomathPreviousSubmission = function () { return {} }
    p5Inst.sendDivomathResult = function () { return sendDivomathResult }
    ...
    let AppStateFlags = Object.entries(defaultAppState).filter(([k, v]) => typeof v === "boolean").map(([k, v]) => k)
    ...
    function fixAppState(AppState, undo = false) {
      let tempAppState = {};
      Object.entries(AppState).forEach(([k, v], i) => {
        let v_ = v;
        if (AppStateFlags.includes(k)) {
          switch (v) {
            case 1: v_ = true; break;
            case "1": v_ = true; break;
            case true: v_ = true; break;
            case "true": v_ = true; break;
            case 0: v_ = false; break;
            case "0": v_ = false; break;
            case "": v_ = false; break;
            case false: v_ = false; break;
            case "false": v_ = false; break;
          }
        } else if (k == "CUBESIZE") {
          v_ = parseInt(v)
        ...
        return tempAppState[k] = v_
      });
      p5Inst.print(tempAppState, tempAppState == AppState, tempAppState === AppState)
      let change = (!undo) ? 1 : -1
      tempAppState["NUMCUBES.COMPACT"] = parseInt(tempAppState["NUMCUBES.COMPACT"]) + change;
      tempAppState["NUMCUBES.SPLIT"] = parseInt(tempAppState["NUMCUBES.SPLIT"]) + change;
      return tempAppState
    }
    ...
    let SEESAWANGLE, SEESAWPIVOTSHIFT;
    ...
    let drawingContexts = new Map([
      ...
      ["scaleDisplayText",
        (B) => {
          B.textAlign(p5Inst.CENTER), B.textSize(AppState["DIGITALSCALETEXTSIZE"]);
    ...
    function drawcubes(buf, cubes, ncubes, compact = false, fullStack = false) {//{{{
      ...
      if (compact) {//{{{
        ...
        if (!fullStack) {
          buf.push();
          let shift = vmult(cubeStackCompactTranslation, -1);
          if (AppMode & AppModes.STACKINGONLYSINGLE) shift = vadd(shift, p5Inst.createVector(-cube.sizex * 0.2, 0));
          if (AppMode & AppModes.STACKINGONLYBOTH) shift = vadd(shift, p5Inst.createVector(-cube.sizex * 0.1, 0));
          buf.translate(...shift.xy);
      ...
      for (var it = 0; it < ncubes; it++) {
        ...
        if (curheight < maxheight) {//{{{
          // fixing display  bug of the sticks slightly hanging over to the left
          if (AppMode & AppModes.STACKINGONLYSPLIT) {
            currentCube.translate(vmult(cube.xEdge, -0.25));
          }
          if (AppMode & AppModes.STACKINGONLYBOTH) {
            currentCube.translate(vmult(cube.xEdge, -0.3));
          }
    ...
    class clickRect extends Clickable {// {{{
      ...
      mouseInArea() {
        ...
        let buffer = p5Inst;
        console.log(this.label, mouseX, mouseY);
        let pos = this.area().slice(0, 2);
        if (this.buffer != null) {
          buffer = this.buffer;
          let bufTrafo = canvasBuffer.rF.T(buffer);
          let [shiftx, shifty] = SEESAWPIVOTSHIFT.xy;
          if ((buffer.__label__ == "cubeStackCompact") ||
            (buffer.__label__ == "cubeStackSplit")) {
            let ROT = new Matrix2D(cos(SEESAWANGLE), -sin(SEESAWANGLE), sin(SEESAWANGLE), cos(SEESAWANGLE)).inv;
            let shiftrot = ROT.dot([mouseX - shiftx, mouseY - shifty]);
            [mouseX, mouseY] = bufTrafo.PP(shiftrot[0] + shiftx, shiftrot[1] + shifty).xy;
          } else {
    ...
    function generateBackgroundImages(cube, cubeStackTranslation, cubeStackBuffers, seesawBoardBufferW) {// {{{
      ...
      if ((bgImgsBase64 == null) || !AppState["USELOCALSTORAGE"]) {
        ...
        for (var label of bgImgLabels) {
          ...
          setTimeout(bgImgsBuffer.pop(), 2000);
      ...
      //LZString.decompress(bgImgsBase64).split("::").forEach((img, i) => {
      //    p5Inst.print("bgImgs", img );
      //});
      return [bgImgsBuffer, {
        split: {
          I: bgImgs[0],
    ...
    class weightSlider {// {{{
      ...
      display(buffer) {
        ...
        for (let [i, y] of this.ticksY.M.entries()) {
          ...
          buffer.textSize(AppState["CENTERSCALETEXTSIZE"]);
    ...
    let seesawSettings = {// {{{
      board: {
        scaleXYZ: [
          ...
          13 /// cubeStackSettings.cube.size,
        ],
        color: undefined
      },
      boardBoth: {
    ...
    AppModes.STACKINGONLYSINGLE = AppModes.STACKINGONLYSPLIT ^ AppModes.STACKINGONLYCOMPACT;
    AppModes.WITHSEESAW = excludeAppModes(AppModes.STACKINGONLYBOTH, AppModes.STACKINGONLYSPLIT, AppModes.STACKINGONLYCOMPACT);
    ...
    p5Inst.preload = function () {
      mylog("begin preload()");
      p5Inst.print("interactive", initialConfig.interactive);
      if (!validOption("RESULT", ["numKG-numG"])) initialConfig.RESULT = null;
      if (!validOption("nocache", [true, false])) initialConfig.nocache = true;
      if (!Number.isInteger(initialConfig.cubesize)) initialConfig.cubesize = 10;
      if (!validOption("interactive", [true, false])) initialConfig.interactive = true;
      ...
      config2SettingIfPresent("seesawboardyoffset", "SEESAWBOARDYOFFSET", (arg) => parseFloat(arg))
      config2SettingIfPresent("digitalscaletextsize", "DIGITALSCALETEXTSIZE", (arg) => parseFloat(arg))
      config2SettingIfPresent("centerscaletextsize", "CENTERSCALETEXTSIZE", (arg) => parseFloat(arg))
      stackingMode = !!(AppModes.STACKING & AppMode);
      config2SettingIfPresent("maxnumcubes", "MAXNUMCUBES", (arg) => parseFloat(arg))
      //if (!isResumed) {



      if (!isResumed) {
        AppState["CUBESIZE"] = initialConfig.cubesize;
    ...
    p5Inst.setup = function () {
      ...
      SEESAWPIVOTSHIFT = p5Inst.createVector(0, 0);
      ...
      CUBESIZE = AppState["CUBESIZE"];

      NUMCUBESLIMIT = canvasBuffer.height * AppState["SEESAWBOARDYOFFSET"] / CUBESIZE / 10;//floor(((0.85 - (0.01 + 2 * canvasBuffer.Rsize(cube).xy.y)) / canvasBuffer.Rsize(cube).xy.y)) * 100;
      NUMCUBESLIMIT = (floor(NUMCUBESLIMIT) + (((NUMCUBESLIMIT - floor(NUMCUBESLIMIT)) > 0.5) ? 0.5 : 0)) * 1000;//floor(((0.85 - (0.01 + 2 * canvasBuffer.Rsize(cube).xy.y)) / canvasBuffer.Rsize(cube).xy.y)) * 100;
      if (AppState["MAXNUMCUBES"]) {
        if (AppState["MAXNUMCUBES"] < NUMCUBESLIMIT) {
          MAXNUMCUBES = AppState["MAXNUMCUBES"];
        }
        else {
          console.warn(`The value of 'maxnumcubes:${AppState["MAXNUMCUBES"]} was to big, capped at ${NUMCUBESLIMIT}'`)
          AppState["MAXNUMCUBES"] = NUMCUBESLIMIT;
        }
      ...
      cube_ = new Cube2D(p5Inst.createVector(-6, 0), cubeStackSettings.cube, "modelCube_");
      ...
      function seesawBoardBufferSetup(seesawBoardScaling, label) {// {{{
        ...
        let seesawBoardBuffersOffset = p5Inst.createVector(
          0.5 - seesawBoardBufferW / 2, AppState["SEESAWBOARDYOFFSET"]
        );
      ...
      cubesSplit = numcubeslike(cubeStackSettings.stack.maxNumCubes, cube_, "split");
      p5Inst.print(cubesSplit);
      mylog("canvas Trafos");
      ...
      let seesawPivotLowerW = AppState["SEESAWBOARDYOFFSET"];//0.8;//TC2SPF.RR(1, 0).x;
      ...
      withContexts(["seesaw"], seesawPivotFrontBuffer)((buf) => {//{{{
        ...
        //AppState["MAXNUMCUBES"] * TC2SPF.PP(10, 10).xy.y;
        ...
        // this adapts the hight to the maxnumcubes
        // @improve: should also be able to change the scaling (single cubes cant be added with sliders yet if ever)
        let height = floor((NUMCUBESLIMIT - AppState["MAXNUMCUBES"]) / 100) * TC2SPF.PR(0, CUBESIZE).y;
        if (AppMode & AppModes.STACKINGNOSCALE) {
          height = AppState["SEESAWBOARDYOFFSET"] - 0.05;//0.75 // same as in the seesawPivotBackBuffer
        };
        let verts = [
          [0.5 - hLowerW, AppState["SEESAWBOARDYOFFSET"] + 0.185],
          [0.5 + hLowerW, AppState["SEESAWBOARDYOFFSET"] + 0.185],
      ...
      withContexts(["seesaw"], seesawPivotBackBuffer)((buf) => {//{{{
        ...
        let height = AppState["SEESAWBOARDYOFFSET"] - 0.05;
        let verts = [
          [0.5 - hLowerW, AppState["SEESAWBOARDYOFFSET"] + 0.185],
          [0.5 + hLowerW, AppState["SEESAWBOARDYOFFSET"] + 0.185],
          [0.5 + hUpperW, heightofboard],
      ...
      let drawerWidth = canvasBuffer.rF.Rx(cubeStackSettings.cube.size) * 11 // 0.125;
      //let drawerWidth = 0.125;
    ...
    p5Inst.draw = function () {
      const _draw = () => {
        ...
        cubeStackSplit.setbackground((DEBUGMODE) ? "#00FF0055" : "#00000000");
        cubeStackCompact.setbackground((DEBUGMODE) ? "#FF000055" : "#00000000");
        ...
        if (numM > 0) {
          ...
          // cubeStackCompact.circle(...xy, 20);
          // cubeStackCompact.circle(xy[0] + w, xy[1], 20);
          // cubeStackCompact.circle(xy[0], xy[1] - h, 20);
          let MStackTarget = new clickRect(...xy, w, -h, cubeStackCompact, "MStackcompact");
          // cubeStackCompact.fill("#00000055");
          // cubeStackCompact.rect(...MStackTarget.area());
        ...
        if (numC > 0) {
          ...
          h = cubeStackSettings.cube.size * (numC + 0.75);
          cubeStackCompact.push();
          cubeStackCompact.fill("#1A19");
          let CStackTarget = new clickRect(...xy, w, -h, cubeStackCompact, "CStackcompact");
          // cubeStackCompact.fill("#55000055");
          // cubeStackCompact.rect(...CStackTarget.area());
          cubeStackCompact.pop();
        }
        if (numI > 0) {
          xy = [verts.IStackEnd[0], verts.IStackEnd[1] - cubeStackSettings.cube.size];
          h = (verts.IStackStart[1] - verts.IStackEnd[1] - cubeStackSettings.cube.size / 2);
          cubeStackCompact.push();
          cubeStackCompact.fill("#11A9");
          let IStackTarget = new clickRect(...xy, w, h, cubeStackCompact, "IStackcompact");
          // cubeStackCompact.fill("#00550055");
          // cubeStackCompact.rect(...IStackTarget.area());
          cubeStackCompact.pop();
        }
        if (numX > 0) {
          xy = [verts.XStackEnd[0], verts.XStackEnd[1] - 2 * cubeStackSettings.cube.size];
          //all rectangles have the same width
          // h = cubeStackSettings.cube.size + cubeStackSettings.cube.zsizey * 10 ;
          h = (verts.XStackStart[1] - verts.XStackEnd[1] + cubeStackSettings.cube.size);
          // p5Inst.print("xy,w,h", xy,w,h)
          cubeStackCompact.push();
          cubeStackCompact.fill("#A1A9");
          let XStackTarget = new clickRect(...xy, w, h, cubeStackCompact, "XStackcompact");
          // cubeStackCompact.fill("#00005555");
          // cubeStackCompact.rect(...XStackTarget.area());
          // cubeStackCompact.circle(...verts.XStackStart, 20)
          // cubeStackCompact.circle(...verts.XStackEnd, 20)
        ...
        if (DEBUGMODE) {
          ...
          Clickable.allClickables.forEach(it => {
            if ((it.constructor.name == "clickRect") && (it.label.endsWith("MStackcompact"))) {
              //console.log(it.area())
            }
          })
    ...
    p5Inst.mousePressed = function () {//{{{
      ...
      p5Inst.print("interactive", AppState.INTERACTIVE);
      if (AppState.INTERACTIVE && canvas.mouseInArea()) {
        Array.from(Clickable.allClickables.values()).forEach(m => m.mousePressed());
        console.log(Clickable.allClickables);
    ...
    p5Inst.mouseReleased = function () {//{{{
      ...
      if (AppState["RESULT"] != null) {
        if (AppState["RESULT"] == "numKG-numG" && (AppMode & AppModes.WITHSEESAW) && !AppState["LOCKSEESAW"]) {
          p5Inst.sendDivomathResult(AppState["NUMCUBES.COMPACT"] - AppState["NUMCUBES.SPLIT"]);
        }
      }
    }//}}}
