const sketch = (p5Inst) => {  // remove const sketch 
  ...
  class Tiles extends Moveable {// {{{
    ...
    static allTiles = [];
    static fillColor = "#DDD";
    static createTiles(location, steps, startDirection = "x", unit = [0.05, 1, "cm"], empty = false, edgelabels = false, frozen = false, zorder = null, resize = true, cuttable = true, label = null) {
      return new Tiles(location, steps, startDirection, unit, empty, edgelabels, frozen, zorder, resize, cuttable, label);
    }

    constructor(location, steps, startDirection = "x", unit = [0.05, 1, "cm"], empty = false, edgelabels = false, frozen = false, zorder = null, resize = true, cuttable = true, label = null) {//{{{
      ...
      this.isVisibel = true;
      this.isActive = false;
      this.fillColor = Tiles.fillColor;
    ...
    snapToTiles() {//{{{
      ...
      if (any(isInside)) {
        ...
        if (enclosingTiles.isEmpty) {
          let color = enclosingTiles.tileColors[enclosingTiles.tileColorIdx];
          enclosingTiles.tileColorIdx = (enclosingTiles.tileColorIdx + 1) % enclosingTiles.tileColors.length;
          ...
          Object.values(this.dragTriangles).forEach(it => {
            ...
            if (!it.isDragged) {
              ...
              if (draggingDirection == "North") {
                ...
                let numCols = abs(this.steps[idx]);
                ...
                let numRows = abs(this.steps[idx]);
  ...
  p5Inst.setup = function() {// {{{
    ...
    if (!validOption(initialConfig, "showresizeeq", [true, false])) initialConfig.showresizeeq = defaultAppState["SHOWRESIZEEQ"];
    if (!validOption(initialConfig.tiles, "cuttable", [true, false])) initialConfig.cuttable = true;
    AppState["SHOWRESIZEEQ"] = initialConfig.showresizeeq;
  ...
  p5Inst.mouseReleased = function() {//{{{
    ...
    if ((draggingTriangle.length == 1)) {
      ...
      let args = [tiles.referencePoint, newSteps, newStartDirection, tiles.unit, tiles.isEmpty, tiles.edgelabels, tiles.isFrozen, tiles.zorder, true, tiles.isCuttable, tiles.label]
